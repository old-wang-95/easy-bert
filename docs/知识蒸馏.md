...
直接finetune的MobileBert，可以达到Bert-Base 99.2%的**效果**、**参数量**小了4倍、**推理速度**快了5.5倍
...
<img height="400" src="images/mobile-bert-diff.png"/>

从上图可知：
- IB-Bert将`521`的hidden加宽到`1024`来近似标准的Bert-Large；
- MobileBert将`512`的hidden变窄到`128`；

MobileBert还做了**一些改进**：
1. **堆了更多的Feed Forward层**，防止FFN的HHA的参数数量比例失衡；
2. 移除了LayerNorm，替换Gelu为Relu激活；
3. embedding层为128，通过kernel size为3的1维卷积转化为512维；

<br>

**MobileBert的损失**：

<img height="80" src="images/mobile-bert-loss.png"/>

- 四项：有监督MLM损失、有监督NSP损失、隐层蒸馏损失、注意力矩阵损失
- 0≤α≤1，权衡 第一项损失 和 最后两项损失，这里取0.5
...
<img height="400" src="images/mobile-bert-distil.png"/>
